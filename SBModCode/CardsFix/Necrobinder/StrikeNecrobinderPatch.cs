...
namespace SBMod.SBModCode.CardsFix.Necrobinder;

[HarmonyPatch(typeof(StrikeNecrobinder))]
public static class StrikeNecrobinderPatch
{
    [HarmonyPatch("CanonicalVars", MethodType.Getter)]
    [HarmonyPostfix]
    static void CanonicalVarsPostfix(StrikeNecrobinder __instance, ref IEnumerable<DynamicVar> __result)
    {
    ...
    [HarmonyPatch("OnPlay")]
    [HarmonyPrefix]
    static bool OnPlayPrefix(StrikeNecrobinder __instance, PlayerChoiceContext choiceContext, CardPlay cardPlay, ref Task __result)
    {
        __result = PatchOnPlay(__instance, choiceContext, cardPlay);
        return false;
    }

    static async Task PatchOnPlay(StrikeNecrobinder instance, PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
    ...
    [HarmonyPrefix]
    static bool OnUpgradePrefix(StrikeNecrobinder __instance)
    {
