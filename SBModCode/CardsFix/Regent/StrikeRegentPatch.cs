...
[HarmonyPatch(typeof(StrikeRegent))]
public static class StrikeRegentPatch
{
    [HarmonyPatch("CanonicalVars", MethodType.Getter)]
    [HarmonyPostfix]
    static void CanonicalVarsPostfix(StrikeRegent __instance, ref IEnumerable<DynamicVar> __result)
    {
    ...
    [HarmonyPatch("OnPlay")]
    [HarmonyPrefix]
    static bool OnPlayPrefix(StrikeRegent __instance, PlayerChoiceContext choiceContext, CardPlay cardPlay, ref Task __result)
    {
        __result = PatchOnPlay(__instance, choiceContext, cardPlay);
        return false;
    }

    static async Task PatchOnPlay(StrikeRegent instance, PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
    ...
    [HarmonyPrefix]
    static bool OnUpgradePrefix(StrikeRegent __instance)
    {
