...
[HarmonyPatch(typeof(StrikeDefect))]
public static class StrikeDefectPatch
{
    [HarmonyPatch("CanonicalVars", MethodType.Getter)]
    [HarmonyPostfix]
    static void CanonicalVarsPostfix(StrikeDefect __instance, ref IEnumerable<DynamicVar> __result)
    {
    ...
    [HarmonyPatch("OnPlay")]
    [HarmonyPrefix]
    static bool OnPlayPrefix(StrikeDefect __instance, PlayerChoiceContext choiceContext, CardPlay cardPlay, ref Task __result)
    {
        __result = PatchOnPlay(__instance, choiceContext, cardPlay);
        return false;
    }

    static async Task PatchOnPlay(StrikeDefect instance, PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
    ...
    [HarmonyPrefix]
    static bool OnUpgradePrefix(StrikeDefect __instance)
    {
