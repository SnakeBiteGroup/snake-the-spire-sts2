...
[HarmonyPatch(typeof(StrikeIronclad))]
public static class StrikeIroncladPatch
{
    [HarmonyPatch("CanonicalVars", MethodType.Getter)]
    [HarmonyPostfix]
    static void CanonicalVarsPostfix(StrikeIronclad __instance, ref IEnumerable<DynamicVar> __result)
    {
    ...
    [HarmonyPatch("OnPlay")]
    [HarmonyPrefix]
    static bool OnPlayPrefix(StrikeIronclad __instance, PlayerChoiceContext choiceContext, CardPlay cardPlay, ref Task __result)
    {
        __result = PatchOnPlay(__instance, choiceContext, cardPlay);
        return false;
    }

    static async Task PatchOnPlay(StrikeIronclad instance, PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
    ...
    [HarmonyPrefix]
    static bool OnUpgradePrefix(StrikeIronclad __instance)
    {
